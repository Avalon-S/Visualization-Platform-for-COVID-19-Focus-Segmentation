...
from SegGroundClassUI import Ui_Form
...
class MainWindow(QWidget, Ui_Form):
    def __init__(self, *args, **kwargs):
        ...
        self.initUI()
        self.setWindowTitle('新冠病灶分割可视化平台')
    ...
    def resetWWWcAndShow(self):
        ...
        try:
            ...
            if objectName == 'cb_wwwc':#下拉框选择窗位
                ...
                self.currWw = self.ww
                self.currWc = self.wc
            elif objectName == 'slider_ww' or objectName == 'slider_wc':#滑块
                self.currWw = self.slider_ww.value()
                self.currWc = self.slider_wc.value()
                self.line_ww.setText(str(self.currWw))
                self.line_wc.setText(str(self.currWc))
            elif objectName == 'line_ww' or objectName == 'line_wc':#文本框输入
